...
The repository provides server-side samples but does not install them automatically. `server/nginx/dict-english.conf` proxies `/dict/english/` to the local API process, while `server/systemd/dict-english.service` defines `DEPLOY_PATH`, reads `$DEPLOY_PATH/.env.dict-english`, and runs `python3 -m vocabdb serve-api` from `$DEPLOY_PATH/backend` (`server/nginx/dict-english.conf:1-17`, `server/systemd/dict-english.service:1-16`, `server/systemd/dict-english.env.example:1-3`).
