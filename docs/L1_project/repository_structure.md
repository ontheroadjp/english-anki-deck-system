...
- `server/`: manually applied VPS configuration sample templates. `server/nginx/` contains the reverse proxy sample, and `server/systemd/` contains the `dict-english` service (`server/nginx/dict-english.conf.example:1-29`, `server/systemd/dict-english.service.example:1-32`).
- `.env.example`: commented template for the server-side `.env` loaded from `$DEPLOY_PATH/.env` (`.env.example:1-12`).
...
- `.github/workflows/ci-cd.yml`: runs `pytest` from `backend/` on pull requests and `main` pushes. On successful `main` pushes, a three-job pipeline (`test` → `build` → `deploy`) builds `backend/vocabulary.db` in CI, uploads a slim payload artifact (`backend/{vocabdb,pyproject.toml,vocabulary.db}`), rsyncs it into `$DEPLOY_PATH/` using `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, and `DEPLOY_PATH` (preserving host-side `.env`, `.venv/`, `server/`, and `backend/*.egg-info/`), bootstraps `.venv` if absent, installs the backend package, and restarts `dict-english` (`.github/workflows/ci-cd.yml:1-130`).
- `server/nginx/dict-english.conf.example`: sample nginx location snippet (intended to be included from a parent `server { ... }` block) that redirects `/dict/english` to `/dict/english/` and proxies `/dict/english/` to the local backend API (`server/nginx/dict-english.conf.example:1-29`).
- `server/systemd/dict-english.service.example`: sample systemd service for the backend API. It defines `DEPLOY_PATH`, loads `$DEPLOY_PATH/.env`, changes into `$DEPLOY_PATH/backend`, and runs `vocabdb serve-api` via `$DEPLOY_PATH/.venv/bin/python` (`server/systemd/dict-english.service.example:1-32`).
- `.env.example`: sample environment values consumed by the systemd service from `$DEPLOY_PATH/.env` (`.env.example:1-12`).
