...
- `.env.example`: commented template for the server-side `.env` loaded from `$DEPLOY_PATH/.env` (`.env.example:1-5`).
...
- `.env.example`: sample environment values consumed by the systemd service from `$DEPLOY_PATH/.env` (`.env.example:1-5`).
