...
- `.github/workflows/ci-cd.yml`: runs `pytest` from `backend/` on pull requests and `main` pushes. On successful `main` pushes, a three-job pipeline (`test` → `build` → `deploy`) builds `backend/vocabulary.db` in CI, uploads a slim payload artifact (`backend/{vocabdb,pyproject.toml,vocabulary.db}`), rsyncs it into `$DEPLOY_PATH/` using `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, and `DEPLOY_PATH` (preserving host-side `.env`, `.venv/`, and `backend/*.egg-info/`), bootstraps `.venv` if absent, installs the backend package, and restarts `dict-english` (`.github/workflows/ci-cd.yml:1-129`).
